...
// 壁の1つ手前のレベルで足踏みしつつ、それ以外は次に進むたびにamountレベルずつ上げる
// (amountはユーザーが設定する「レベルアップ幅」。壁の手前を超えないようclampする)。
export function decideAutoLevel(
  currentLevel: number,
  ceiling: number | null,
  amount: number,
  clamp: (n: number) => number
): number {
  if (ceiling === null) return clamp(currentLevel + amount);
  const cap = ceiling - 1;
  if (currentLevel >= cap) return currentLevel;
  return clamp(Math.min(currentLevel + amount, cap));
}
